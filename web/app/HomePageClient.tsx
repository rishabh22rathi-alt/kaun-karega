...
// Minimal shape for an entry in the "show me the provider list directly"
// fallback panel. The list is hydrated from /api/find-provider; fields
// are intentionally optional because the legacy endpoint shape is loose.
...
// JSON shape returned by /api/submit-request and /api/submit-approval-request
// — fields are typed as `unknown` because the server has historically
// returned mixed shapes; the call sites narrow via typeof checks or
// optional-chain truthy fallbacks before reading.
type SubmitResponseShape = {
  ok?: unknown;
  error?: unknown;
  message?: unknown;
  displayId?: unknown;
...
function PageContent() {
  ...
  const [providersList, setProvidersList] = useState<HomeProviderRow[]>([]);
  ...
  useEffect(() => {
    ...
    // Intentionally NOT including `category` — this effect is a one-way
    // bootstrap from the URL into state, gated on
    // userPickedCategoryRef.current so a stale URL param can't override a
    // value the user actively typed/selected later. Reacting to `category`
    // would either no-op (guard true) or trigger a setCategory loop on
    // re-mount; including it just makes the effect re-fire pointlessly
    // on every keystroke. Same pattern as the typewriter effect at the
...
const submitResolvedRequest = async (resolution: CategoryResolution) => {
  ...
  if (!resolution.isConfident) {
    try {
      ...
      let approvalJson: SubmitResponseShape | null = null;
      try {
        approvalJson = JSON.parse(rawApproval) as SubmitResponseShape;
      } catch {}

      if (!res.ok || !approvalJson?.ok) {
        const approvalMessage =
          typeof approvalJson?.message === "string"
            ? approvalJson.message
            : "";
        const approvalErrorText =
          typeof approvalJson?.error === "string"
            ? approvalJson.error
            : "";
        throw new Error(
          approvalMessage ||
      ...
      const approvalDisplayId =
        typeof approvalJson?.displayId === "string"
          ? approvalJson.displayId.trim()
          : "";
      const approvalTaskId =
        typeof approvalJson?.taskId === "string"
          ? approvalJson.taskId.trim()
          : "";
      const refId =
        approvalDisplayId ||
        approvalTaskId ||
  ...
  try {
    ...
    let json: SubmitResponseShape | null = null;
    try {
      json = JSON.parse(raw) as SubmitResponseShape;
    } catch {}
    ...
    // Narrow the unknown-typed body fields once so the string-only
    // setError() / buildSuccessRedirect() call sites stay type-safe
    // after replacing the historic `any` body type.
    const bodyMessage =
      typeof json?.message === "string" ? json.message : "";
    const bodyError = typeof json?.error === "string" ? json.error : "";
    const bodyTaskId = typeof json?.taskId === "string" ? json.taskId : "";
    const bodyDisplayId =
      typeof json?.displayId === "string" ? json.displayId : "";

    if (!res.ok) {
      setError(bodyMessage || bodyError || "Non-200 response");
      return;
    }

    if (!json?.ok) {
      setError(bodyMessage || bodyError || "ok=false");
      return;
    }
...
const hasArea = area.trim() !== "";
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* ── PROVIDER CTA ──────────────────────────────────────────────
          Phase 1-B refinement (Devanagari pass): same section, same
          dark-gradient design, same /provider/register route — only
          the copy was rewritten in natural Hindi (Devanagari) so
          local Jodhpur providers feel addressed in their own language.
          Bullets mix English for the platform mechanics ("Free
          registration to start", "provider dashboard") with Hindi
          action verbs so the everyday actions a provider takes read
          naturally. Section visibility gate (!hasCategory) is
          unchanged — it hides as soon as the user starts a task.
      */}
      {!hasCategory && (
        <section
          ...
          onDragStart={(e) => e.preventDefault()}
        >
          <div className="mx-auto max-w-2xl sm:flex sm:items-start sm:gap-10">

            {/* Left: copy */}
            <div className="flex-1 text-center sm:text-left">
              {/* Eyebrow pill — cream background with forest-green
                  text avoids the poor contrast of forest-green
                  directly on the dark gradient. Cream token
                  (#FFE3C2) matches the "Coming Soon" pill on the
                  paused /i-need page so the brand vocabulary stays
                  consistent. inline-flex lets the parent's
                  text-center/sm:text-left alignment continue to
                  work on the span. */}
              <span className="inline-flex rounded-full bg-[#FFE3C2] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[#003d20]">
              ...
              >
                बनिए उस सवाल का जवाब,
                <br />
                जब कोई पूछे — ये काम कौन करेगा?
              </h2>
              <p
                lang="hi"
                className="mx-auto mt-3 max-w-xl text-sm leading-7 text-white/90 sm:mx-0"
              >
                {/* Visual emphasis on the city: "जोधपुर" is one step
                    larger (text-lg → sm:text-xl), brand-orange, and
                    semibold so it pops against the dark gradient.
                    "वासी" stays at the surrounding white/90 weight.
                    Orange token matches the bullet checkmarks below
                    (text-orange-400) for in-section consistency. */}
                <span className="text-lg font-semibold text-orange-400 sm:text-xl">
                  जोधपुर
                </span>
                {" "}
                <span className="font-normal text-white/90">वासी</span>
                {" "}
                आपके हुनर और आपके काम को रोज़ाना ढूंढ रहे हैं।
              </p>
              <p
                lang="hi"
                className="mx-auto mt-3 max-w-xl text-sm leading-7 text-white/90 sm:mx-0"
              >
                हो सकता है जिन्हें आपकी ज़रूरत है, वे आपको ढूंढ न पा रहे
                हों। आज ही{" "}
                {/* Brand-orange highlight on "Kaun Karega" so the
                    product name pops in-line with the city emphasis
                    above; same orange-400 token. */}
                <span className="font-semibold text-orange-400">
                  Kaun Karega
                </span>
                {" "}पर अपने किसी भी प्रकार के काम, हुनर या सर्विस को
                रजिस्टर करें — जहाँ हम आपको उन लोगों से मिलाते हैं
                जिन्हें सच में आपकी ज़रूरत है।
              </p>

              {/* Benefit bullets — mixed English + Hindi as designed. */}
              <ul className="mx-auto mt-4 max-w-xl space-y-1.5 text-left text-sm text-slate-300 sm:mx-0">
                {[
                  "Free registration to start",
                  "अपनी सर्विस और एरिया चुनें",
                  "लोकल ग्राहकों से काम की रिक्वेस्ट पाएं",
                  "Leads को provider dashboard से manage करें",
                ].map((b) => (
            ...
            {/* Right: CTA button */}
            <div className="mt-7 text-center sm:mt-0 sm:shrink-0 sm:self-center">
              <button
                type="button"
              ...
              <p className="mt-2 text-xs text-slate-400">
                Free registration. No commitment.
              </p>
            </div>
          </div>
        </section>
