...
import {
  describeProviderTestPushResult,
  providerWebDeviceBody,
  registerProviderPush,
  type ProviderTestPushMessage,
} from "@/lib/push/providerPushCore";

/**
 * Provider Push Phase 1 — provider web-push registration hook.
 *
 ...
 *
 * Self-contained mirror of useAdminPush so the admin path stays untouched.
 */

export type ProviderPushPermission = NotificationPermission | "unsupported";
...
// nothing was delivered (handled by describeProviderTestPushResult).
const REGISTERED_KEY = "kk.providerPush.registered";
...
export function useProviderPush() {
  ...
  const permission: ProviderPushPermission = supported
  ...
  const [testMessage, setTestMessage] = useState<ProviderTestPushMessage | null>(
    null
  );
  ...
  const register = useCallback(async () => {
    ...
    const outcome = await registerProviderPush({
      ...
      postDevice: async (token) => {
        try {
          const res = await fetch("/api/native-push/devices", {
            ...
            body: JSON.stringify(providerWebDeviceBody(token)),
          });
  ...
  // Send a test push to THIS provider's own registered web device(s).
  // Never throws; result is surfaced as a message. No business event.
  const sendTest = useCallback(async () => {
    setTestLoading(true);
    setTestMessage(null);
    try {
      const res = await fetch("/api/provider/push/test", {
        method: "POST",
        credentials: "same-origin",
      });
      const data = (await res.json().catch(() => null)) as
        ...
        | null;
      setTestMessage(describeProviderTestPushResult(res.ok, data));
    } catch {
